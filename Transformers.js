(function () {
  var DEF = { a: 1, b: 0, c: 0, d: 1, tx: 0, ty: 0 };
  ...
  Transformers.prototype = {
     ...
     *
     * mat.multiply({ a: 1, b: 0, c: 0, d: 1, tx: 0, ty: 0 });
     * mat.multiply([1, 0, 0, 1, 0, 0]);
     * mat.multiply(mat); // { a: 1, b: 0, c: 0, d: 1, tx: 0, ty: 0 }
     */
    multiply: function (mat) {
      var matrix = Object.assign({}, DEF);

      if (mat instanceof Transformers) {
        mat = mat.matrix;
      }
      if (Array.isArray(mat)) {
        mat = array2Matrix(mat);
      }
      ...
      matrix.tx = this.matrix.a * mat.tx + this.matrix.c * mat.ty + this.matrix.tx;
      matrix.ty = this.matrix.b * mat.tx + this.matrix.d * mat.ty + this.matrix.ty;
     ...
     * var mat = transformers();
     *
     * mat.parse('translate(10,10)'); // {a: 1, b: 0, c: 0, d: 1, tx: 10, ty: 10}
     */
    parse: function (str) {
      ...
      if (typeof str === "string") {
        while ((match = regex.exec(str))) {
          ...
          switch (tran) {
            case "translate":
              this.translate(vals[0], vals[1]);
              break;
            case "rotate":
              this.rotate(vals[0], vals[1], vals[2]);
              break;
            case "scale":
              this.scale(vals[0], vals[1]);
              break;
            case "shear":
              this.shear(vals[0], vals[1]);
              break;
            case "skew":
              this.skew(vals[0], vals[1]);
              break;
            case "matrix":
              this.multiply({
                a: vals[0],
     ...
     *
     * mat.translate(); // {a: 1, b: 0, c: 0, d: 1, tx: 10, ty: 10}
     * mat.translate(5, 5); // {a: 1, b: 0, c: 0, d: 1, tx: 15, ty: 15}
     */
    translate: function (x = 0, y = 0) {
      var mat = { a: 1, b: 0, c: 0, d: 1, tx: x, ty: y };
    ...
    rotate: function (angle, x = 0, y = 0) {
      var cosAngle, sinAngle, mat;

      angle = (angle * Math.PI) / 180;
      cosAngle = Math.cos(angle);
      sinAngle = Math.sin(angle);
      mat = { a: cosAngle, b: sinAngle, c: -sinAngle, d: cosAngle, tx: 0, ty: 0 };
     ...
     * var mat = transformers('translate(10, 10)');
     *
     * mat.scale(5); // {a: 5, b: 0, c: 0, d: 5, tx: 10, ty: 10}
     * mat.scale(5, 2); // {a: 5, b: 0, c: 0, d: 2, tx: 10, ty: 10}
     */
    scale: function (x, y) {
      var mat;
     ...
     * var mat = transformers();
     *
     * mat.shear(5,5); // {a: 1, b: 5, c: 5, d: 1, tx: 0, ty: 0}
     */
    shear: function (x, y) {
      var mat = { a: 1, b: y, c: x, d: 1, tx: 0, ty: 0 };
     ...
     * var mat = transformers();
     *
     * mat.skew(5,5); // {a: 1, b: -3.3805, c: -3.3805, d: 1, tx: 0, ty: 0}
     */
    skew: function (x, y) {
      var mat = { a: 1, b: Math.tan(y), c: Math.tan(x), d: 1, tx: 0, ty: 0 };
     ...
     * var mat = transformers('translate(10, 10)');
     *
     * mat.inverse(); // {a: 1, b: 0, c: 0, d: 1, tx: -10, ty: -10}
     */
    inverse: function () {
    ...
    pointTo: function (x = 0, y = 0) {
      ...
      return { x: mat.a * x + mat.c * y + mat.tx, y: mat.b * x + mat.d * y + mat.ty };
    },
    /**
  ...
  function array2Matrix(arr) {
    return { a: arr[0], b: arr[1], c: arr[2], d: arr[3], tx: arr[4], ty: arr[5] };
  }
